...
export function useTelegramConnectGate() {
  ...
  const requireTelegramLinkedWallet = useCallback(async (options: { force?: boolean } = {}) => {
    if (!isConnected) {
      openWalletModal();
      throw new Error("Choose a wallet to continue.");
    }

    const wallet = await getWalletAuth({ force: options.force });
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="sm:max-w-[25rem]">
        <DialogHeader>
          ...
          <DialogDescription>
            Link Telegram to receive Mantle alpha alerts before running
            Langclaw.
          </DialogDescription>
        </DialogHeader>
